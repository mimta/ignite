...
ARG VERSION
WORKDIR /go/src/github.com/mimta/ignite
COPY . .
...
WORKDIR /root/ignite
COPY --from=builder /go/src/github.com/mimta/ignite ./
COPY --from=builder /go/src/github.com/mimta/ignite/templates ./templates
COPY --from=builder /go/src/github.com/mimta/ignite/static ./static
COPY --from=builder /go/src/github.com/mimta/ignite/conf ./conf
